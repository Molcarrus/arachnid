...
impl Plugin for TerrainPlugin {
    fn build(&self, app: &mut App) {
        app
            .init_resource::<CurrentTerrain>()
            .add_event::<TerrainChangeEvent>()
            .add_systems(Startup, setup_terrain)
            .add_systems(Update, (
                handle_terrain_change,
                update_terrain_collider_cache,
            ));
    }
}
...
fn setup_terrain(
    ...
    spawn_terrain(TerrainType::Flat, &mut commands, &mut meshes, &mut materials);
}
...
fn handle_terrain_change(
    ...
    for event in events.read() {
        ...
        spawn_terrain(event.new_terrain, &mut commands, &mut meshes, &mut materials);
    }
}
...
fn spawn_terrain(
    ...
    match terrain_type {
        TerrainType::Flat => {
            spawn_box(commands, meshes, &ground_mat, Vec3::new(0.0, -0.1, 0.0), Vec3::new(200.0, 0.2, 200.0));
        }
        ...
        TerrainType::Hills => {
            ...
            for (center, radius, max_height) in &hill_positions {
                ...
                for i in 0..layers {
                    ...
                    spawn_box(commands, meshes, &accent_mat, Vec3::new(center.x, y, center.z), Vec3::new(layer_radius * 2.0, layer_height, layer_radius * 2.0));
                }
            }
        }
        ...
        TerrainType::Steps => {
            ...
            for i in 0..step_count {
                ...
                spawn_box(commands, meshes, &accent_mat, Vec3::new(x, height / 2.0, 0.0), Vec3::new(step_depth, height, step_depth * 2.0));
            }
            ...
            for i in 0..step_count {
                ...
                spawn_box(commands, meshes, &accent_mat, Vec3::new(-5.0, height / 2.0, z), Vec3::new(step_width * 2.0, height, step_depth));
            }
        }
        ...
        TerrainType::Ramps => {
            ...
            for i in 0..ramp_slices {
                ...
                spawn_box(commands, meshes, &accent_mat, Vec3::new(x, h / 2.0, 6.0), Vec3::new(slice_length, h, ramp_width));
            }
            
            spawn_box(commands, meshes, &accent_mat, Vec3::new(ramp_length + 2.0, ramp_height / 2.0, 6.0), Vec3::new(4.0, ramp_height, ramp_width));
            ...
            for i in 0..ramp_slices {
                ...
                spawn_box(commands, meshes, &accent_mat, Vec3::new(-6.0, h / 2.0, z), Vec3::new(ramp_width, h, slice_length));
            }
        }
        ...
        TerrainType::Rocky => {
            ...
            for (pos, size) in &rocks {
                let y = size.y / 2.0;
                spawn_box(commands, meshes, &rock_mat, Vec3::new(pos.x, y, pos.z), *size);
            }
        }
    }
}
