...
use crate::kin::{self, IkChain};

const TARGET_RADIUS: f32 = 0.7;
const TARGET_COLOR: Color = Color::Srgba(css::ORANGE_RED);
const CURRENT_TARGET_COLOR: Color = Color::Srgba(css::LIMEGREEN);

const FARBIC_ITERATIONS: i32 = 6;
...
impl Plugin for IkLegPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(
            Update,
            (draw_animated_leg_gizmos, animate_leg_towards_target),
        );
    }
}
...
fn animate_leg_towards_target(
    ...
    for (mut chain, mut leg) in animated_legs.iter_mut() {
        leg.increase_lerp_fraction(LERP_SPEED * time.delta_secs());

        let start = leg.previous_target;
        let end = leg.current_target;
        let distance = start.distance(end);
        let curve_anchor = start.lerp(end, 0.5) + Vec3::Y * distance * CURVE_HEIGHT;
        ...
        kin::solve_chain_towards_target(
            &mut chain,
            interpolated_target,
            FARBIC_ITERATIONS,
            &mut gizmos,
...
fn draw_animated_leg_gizmos(mut gizmos: Gizmos, animated_legs: Query<(&IkChain, &AnimatedLeg)>) {
    if DRAW_TARGET_GIZMOS {
        for (chain, leg) in animated_legs.iter() {
            ...
            draw_target(&mut gizmos, leg.current_target, TARGET_RADIUS, TARGET_COLOR);
        }
    }
}

fn draw_target(gizmos: &mut Gizmos, position: Vec3, radius: f32, color: Color) {
    gizmos.circle(position, 0.1, color);
